...
function showLoginRequiredPopup(action = 'perform this action') {
    let loginPopup = document.getElementById('loginRequiredPopup');
    
    if (!loginPopup) {
        loginPopup = document.createElement('div');
        loginPopup.id = 'loginRequiredPopup';
        loginPopup.className = 'modal';
        loginPopup.innerHTML = `
            <div class="modal-content">
                <h3>Login Required</h3>
                <p id="loginRequiredMessage">Please log in to ${action}.</p>
                <button onclick="redirectToLogin()" class="confirm-btn">Login</button>
                <button onclick="closeLoginRequiredPopup()" class="cancel-btn">Cancel</button>
            </div>
        `;
...
function addToCart() {
    // Check if user is logged in
    if (!isUserLoggedIn()) {
        showLoginRequiredPopup('add items to cart');
        return;
    }
...
function buyNow() {
    // Check if user is logged in
    if (!isUserLoggedIn()) {
        showLoginRequiredPopup('purchase items');
        return;
    }
...
function proceedToCheckout() {
    // Check if user is logged in
    if (!isUserLoggedIn()) {
        showLoginRequiredPopup('proceed to checkout');
        return;
    }
