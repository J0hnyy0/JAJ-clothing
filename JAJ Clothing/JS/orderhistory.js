...
function displayOrders(snapshot) {
      ...
      <tr>
        <td colspan="6" style="text-align:center; padding: 40px; color:#999;">
          <div class="empty-state">
            ...
            <p>Your confirmed orders will appear here once processed by our admin.</p>
            <p><small>Orders with "pending" status are being reviewed and will show here after confirmation.</small></p>
  ...
  snapshot.forEach(doc => {
    ...
    let row = `
      <tr class="order-row" )">
        ...
        <td>
          <button class="btn-details" onclick="event.stopPropagation(); showOrderDetails('${doc.id}')">
            <i class="ri-eye-line"></i> Details
          </button>
        </td>
      </tr>
      <tr class="order-details-row" id="details-${doc.id}" style="display: none;">
        <td colspan="6">
          <div class="order-details-container">
            ...
            <div class="order-timeline">
              ...
              <div class="timeline">
                <div class="timeline-item ${order.status === 'confirmed' || ['processing', 'shipped', 'delivered'].includes(order.status) ? 'completed' : ''}">
                  <div class="timeline-marker"><i class="ri-check-double-line"></i></div>
                  <div class="timeline-content">
                    <div class="timeline-title">Order Confirmed</div>
                    <div class="timeline-date">${order.confirmedAt ? new Date(order.confirmedAt.seconds * 1000).toLocaleString() : 'Confirmed'}</div>
                  </div>
                </div>
                <div class="timeline-item ${['processing', 'shipped', 'delivered'].includes(order.status) ? 'completed' : ''}">
                  ...
                  <div class="timeline-content">
                    ...
                    <div class="timeline-date">${order.status === 'processing' || ['shipped', 'delivered'].includes(order.status) ? 'In progress' : 'Pending'}</div>
                  </div>
                </div>
  ...
  const pageTitle = document.getElementById('page-title');
  if (pageTitle) {
    pageTitle.textContent = `Order History (${snapshot.size} Orders)`;
  }
}
    ...
    <tr>
      <td colspan="6" style="text-align:center; padding: 40px; color:#999;">
        <div class="empty-state">
          ...
          <p><small>All your confirmed orders will be displayed here once you're logged in.</small></p>
