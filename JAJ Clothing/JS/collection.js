// collection.js (updated with new checkout functions)
...
function selectProduct(productData) {
    // Remove duplicate items from localStorage cart
    ...
    // Save selected product and go to product page
    localStorage.setItem('selectedProduct', JSON.stringify(productData));
    window.location.href = `Product.html?id=${productData.id}`;
}
...
function logout() {
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
        firebase.auth().signOut()
            .then(() => {
                // Clear any local storage state as a fallback
            ...
            .catch((error) => {
                ...
                // Optionally display a visible error in the UI instead of alert
            });
    } else {
